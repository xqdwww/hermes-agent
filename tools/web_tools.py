...
WEB_EXTRACT_SCHEMA = {
    "name": "web_extract",
    "description": "Extract content from web page URLs. Returns clean page content in markdown/text (no LLM summarization — fast). Also works with PDF URLs (arxiv papers, documents) — pass the PDF link directly. Pages within the char budget (default 15000) return whole; larger pages return a head+tail window with a footer telling you the full text's saved file path and the read_file call to page through the omitted middle. Inline images appear as [IMAGE: alt] placeholders; real image URLs are kept as links. If extraction fails (timeout, blocked, error), report the failure to the user — do NOT fall back to research_pipeline_runner, task_engine_runner, or terminal curl. IMPORTANT: Do NOT use for video URLs (b23.tv, bilibili.com/video, youtube.com/watch, youtu.be) — those must be routed to the summarizer video pipeline, not web_extract. web_extract will be blocked by the research gate for non-HTTP content or may return empty metadata. For Bilibili URLs (bilibili.com, b23.tv), this tool returns only basic page metadata (title, author, duration, publish date, stats) — the description field is often empty and subtitles/transcripts are NOT included. Do NOT claim you have extracted the actual video content when only metadata is available. Do NOT invoke youtube-content or other video-processing skills for Bilibili URLs.",
    "parameters": {
        "type": "object",
        "properties": {
            "urls": {
