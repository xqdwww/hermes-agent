...
It runs the helper against a private configuration clone and atomically commits
only the refreshed source/index files, so helper initialization cannot rewrite
the running app's configuration. It never replaces, restarts, or reconfigures
the installed app. Version 2.5.0 adds resumable candidate preparation from one
...
6. Capture active profile, proxy mode, selected-node state, effective system proxy, TUN setting, Mihomo process state, and network exit before refresh. Require the same values afterward. Copy only the required config/profile/GeoIP inputs into a private mode-0700 helper home, run the temporary helper there, and atomically commit only the refreshed profile plus `profiles.yaml` after proving both live files are unchanged. Whether the normal app is running or stopped, wait for authenticated READY, refresh, freeze, request authenticated shutdown, verify its dedicated port closes, and remove the helper home. Never restore unrelated live config bytes from an earlier snapshot because that could erase a legitimate concurrent app change.
