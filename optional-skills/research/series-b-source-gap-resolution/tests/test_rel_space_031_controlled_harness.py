#!/usr/bin/env python3
"""Smoke and contract tests for the rel_space_031 controlled harness."""
...
def run_tests():
    with tempfile.TemporaryDirectory(prefix="rel-space-031-harness-") as tmp_str:
        tmp=Path(tmp_str)
        ...
        assert payload["term_coverage"]["Shinto"]["passed"] is True
        assert payload["term_coverage"]["torii"]["passed"] is True
        assert payload["term_coverage"]["shimenawa"]["passed"] is True
...
if __name__ == "__main__":
    run_tests()
    print("rel_space_031 controlled harness tests PASS")
