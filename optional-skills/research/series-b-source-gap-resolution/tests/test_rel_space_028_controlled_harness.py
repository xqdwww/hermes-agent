#!/usr/bin/env python3
"""Smoke and contract tests for the rel_space_028 controlled harness."""
...
def run_tests():
    with tempfile.TemporaryDirectory(prefix="rel-space-028-harness-") as tmp_str:
        tmp=Path(tmp_str)
        completed,payload=_runner(_base_args(tmp/"dry"))
        assert completed.returncode == 0, payload
        assert payload["approved_chunks_count"] == 53
        ...
        assert payload["term_coverage"]["chaitya"]["passed"] is True
        assert payload["term_coverage"]["stupa"]["passed"] is True
...
if __name__ == "__main__":
    run_tests()
    print("rel_space_028 controlled harness tests PASS")
