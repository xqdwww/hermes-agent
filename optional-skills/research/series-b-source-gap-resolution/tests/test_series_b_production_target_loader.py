...
def test_official_baseline_55_and_prior_traces_retained() -> None:
    validation = validate_production_target_manifest(DEFAULT_TARGET_MANIFEST)
    baseline = json.loads(Path(validation["resolved_paths"]["official_baseline_file"]).read_text(encoding="utf-8"))
    assert baseline["official_score"] == "55/60"
    assert baseline["prior_score"] == "50/60"
    assert baseline["production_default_integrated"] is False
    assert baseline["controlled_evidence_count"] == 28
...
def run_tests() -> None:
    ...
    test_official_baseline_55_and_prior_traces_retained()
