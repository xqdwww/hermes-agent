#!/usr/bin/env python3
"""Smoke and contract tests for the nat_eco_041 controlled harness."""
...
from series_b_nat_eco_041_artifact_contract import validate_nat_eco_041_artifact_contract
from series_b_nat_eco_041_result_schema import REQUIRED_ARTIFACTS
...
HANDOFF = Path(
    ...
    "travel-series-b-nat-eco-041-controlled-planning-handoff/outputs/"
    "nat_eco_041_controlled_handoff_manifest.json"
)
CHUNKS = Path(
    "/Users/xqdwww/Documents/Codex/2026-06-25/"
    "travel-series-b-nat-eco-041-controlled-planning-handoff/outputs/"
    "nat_eco_041_approved_chunks_handoff.json"
)
RUNNER = TOOLS_DIR / "run_nat_eco_041_single_case_controlled.py"
...
def run_tests() -> None:
    with tempfile.TemporaryDirectory(prefix="nat-eco-041-harness-") as tmp_str:
        tmp = Path(tmp_str)

        exported = export_nat_eco_041_controlled_source_packet(
            case_id="nat_eco_041",
        ...
        assert packet["case_id"] == "nat_eco_041"
        assert packet["caveats"]["binding_caveat_preserved"] is True
        assert packet["caveats"]["dune_migration_caveat_preserved"] is True
        assert len(packet["professional_chunks"]) == 13
        assert len(packet["context_chunks"]) == 28
        assert all(chunk["binding_status"] == "PARTIAL_SECTION_LOCATOR_USED" for chunk in packet["professional_chunks"])
        ...
        assert payload["dune_migration_caveat_preserved"] is True
        assert payload["term_coverage"]["saltation"]["passed"] is True
        contract = validate_nat_eco_041_artifact_contract(tmp / "real")
        ...
        bad_chunks["professional_chunks"][0]["sports_noise"] = True
        _assert_error(
            "SOURCE_PACKET_CONTAMINATION_DETECTED",
            export_nat_eco_041_controlled_source_packet,
            case_id="nat_eco_041",
            approved_chunks_handoff_path=_write_json(tmp, "bad_sports.json", bad_chunks),
            controlled_handoff_manifest_path=HANDOFF,
        ...
        bad_chunks = copy.deepcopy(json.loads(CHUNKS.read_text(encoding="utf-8")))
        bad_chunks["professional_chunks"][0]["binding_status"] = "TRUE_PAGE_BOUND"
        _assert_error(
        ...
        completed, payload = _runner_json([*_base_args(tmp / "bad-prod"), "--execute-real-controlled-dry-run"])
...
if __name__ == "__main__":
    run_tests()
    print("nat_eco_041 controlled harness tests PASS")
