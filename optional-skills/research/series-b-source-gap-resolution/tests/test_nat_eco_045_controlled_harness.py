#!/usr/bin/env python3
"""Smoke and contract tests for the nat_eco_045 controlled harness."""
...
def run_tests():
    with tempfile.TemporaryDirectory(prefix="nat-eco-045-harness-") as tmp_str:
        tmp=Path(tmp_str)
        completed,payload=_runner(_base_args(tmp/"dry"))
        assert completed.returncode == 0, payload
        assert payload["approved_chunks_count"] == 21
        ...
        assert payload["term_coverage"]["cirque"]["passed"] is True
        assert payload["term_coverage"]["tarn"]["passed"] is True
        assert payload["axis_coverage"]["nature_book"] is True
        assert payload["axis_coverage"]["external_book"] is True
...
if __name__ == "__main__":
    run_tests()
    print("nat_eco_045 controlled harness tests PASS")
