#!/usr/bin/env python3
"""Smoke and contract tests for the nat_eco_046 controlled harness."""
...
def run_tests():
    with tempfile.TemporaryDirectory(prefix="nat-eco-046-harness-") as tmp_str:
        tmp=Path(tmp_str)
        completed,payload=_runner(_base_args(tmp/"dry"))
        assert completed.returncode == 0, payload
        assert payload["approved_chunks_count"] == 35
        ...
        assert payload["term_coverage"]["Loess"]["passed"] is True
        assert payload["term_coverage"]["erosion"]["passed"] is True
...
if __name__ == "__main__":
    run_tests()
    print("nat_eco_046 controlled harness tests PASS")
