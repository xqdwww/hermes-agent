#!/usr/bin/env python3
"""Smoke and contract tests for the obj_art_011 controlled harness."""
...
def run_tests():
    with tempfile.TemporaryDirectory(prefix="obj-art-011-harness-") as tmp_str:
        tmp=Path(tmp_str)
        completed,payload=_runner(_base_args(tmp/"dry"))
        assert completed.returncode == 0, payload
        assert payload["approved_chunks_count"] == 61
        ...
        assert payload["term_coverage"]["Corinthian"]["passed"] is True
        assert payload["term_coverage"]["capital"]["passed"] is True
        assert payload["axis_coverage"]["art_architecture_book"] is True
...
if __name__ == "__main__":
    run_tests()
    print("obj_art_011 controlled harness tests PASS")
