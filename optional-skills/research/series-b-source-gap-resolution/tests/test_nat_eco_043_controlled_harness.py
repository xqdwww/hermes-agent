#!/usr/bin/env python3
"""Smoke and contract tests for the nat_eco_043 controlled harness."""
...
def run_tests():
    with tempfile.TemporaryDirectory(prefix="nat-eco-043-harness-") as tmp_str:
        tmp=Path(tmp_str)
        completed,payload=_runner(_base_args(tmp/"dry"))
        assert completed.returncode == 0, payload
        assert payload["approved_chunks_count"] == 59
        ...
        assert payload["term_coverage"]["alluvial"]["passed"] is True
        assert payload["term_coverage"]["sediment"]["passed"] is True
        assert payload["axis_coverage"]["nature_book"] is True
...
if __name__ == "__main__":
    run_tests()
    print("nat_eco_043 controlled harness tests PASS")
