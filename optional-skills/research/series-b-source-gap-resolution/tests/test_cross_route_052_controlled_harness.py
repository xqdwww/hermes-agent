#!/usr/bin/env python3
"""Smoke and contract tests for the cross_route_052 controlled harness."""
...
from series_b_cross_route_052_artifact_contract import validate_cross_route_052_artifact_contract
from series_b_cross_route_052_result_schema import REQUIRED_ARTIFACTS
...
def run_tests() -> None:
    with tempfile.TemporaryDirectory(prefix="cross-route-052-harness-") as tmp_str:
        tmp = Path(tmp_str)

        exported = export_cross_route_052_controlled_source_packet(
        ...
        assert packet["case_id"] == "cross_route_052"
        assert packet["caveats"]["binding_caveat_preserved"] is True
        assert packet["caveats"]["hydrology_caveat_preserved"] is True
        assert packet["caveats"]["hydrology_primary_evidence"] is False
        assert len(packet["chunks"]) == 14
        assert len(packet["context_or_alias_guard_chunks"]) == 3
        ...
        assert payload["term_coverage"]["hydrology"]["passed"] is True
        assert payload["term_coverage"]["cliff stability"]["passed"] is True
        contract = validate_cross_route_052_artifact_contract(tmp / "real")
        ...
        bad_chunks = copy.deepcopy(json.loads(CHUNKS.read_text(encoding="utf-8")))
        bad_chunks["approved_primary_chunks"][0]["supports_terms"].append("hydrology")
        _assert_error(
            "SOURCE_PACKET_CONTAMINATION_DETECTED",
            export_cross_route_052_controlled_source_packet,
            case_id="cross_route_052",
            approved_chunks_handoff_path=_write_json(tmp, "bad_hydrology.json", bad_chunks),
            controlled_handoff_manifest_path=HANDOFF,
            output_dir=tmp / "bad-hydrology",
            no_production_default=True,
            no_baseline_update=True,
            no_full_series_b=True,
            repo_root=REPO_ROOT,
        ...
        bad_chunks = copy.deepcopy(json.loads(CHUNKS.read_text(encoding="utf-8")))
        bad_chunks["approved_supplemental_chunks"][0]["binding_status"] = "PAGE_PRECISE"
        _assert_error(
            "SOURCE_PACKET_CONTAMINATION_DETECTED",
            export_cross_route_052_controlled_source_packet,
            case_id="cross_route_052",
...
if __name__ == "__main__":
    run_tests()
    print("cross_route_052 controlled harness tests PASS")
