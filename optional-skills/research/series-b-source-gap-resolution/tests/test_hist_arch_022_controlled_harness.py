#!/usr/bin/env python3
"""Smoke and contract tests for the hist_arch_022 controlled harness."""
...
def run_tests():
    with tempfile.TemporaryDirectory(prefix="hist-arch-022-harness-") as tmp_str:
        tmp=Path(tmp_str)
        completed,payload=_runner(_base_args(tmp/"dry"))
        assert completed.returncode == 0, payload
        assert payload["approved_chunks_count"] == 192
        ...
        assert payload["term_coverage"]["Grand Canal"]["passed"] is True
        assert payload["term_coverage"]["caozhi"]["passed"] is True
        assert payload["term_coverage"]["lock"]["passed"] is True
        assert payload["term_coverage"]["hydraulic"]["passed"] is True
        assert payload["axis_coverage"]["history_book"] is True
        assert payload["axis_coverage"]["qyer_or_china_local"] is True
...
if __name__ == "__main__":
    run_tests()
    print("hist_arch_022 controlled harness tests PASS")
