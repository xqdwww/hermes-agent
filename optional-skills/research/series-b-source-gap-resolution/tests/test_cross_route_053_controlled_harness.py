#!/usr/bin/env python3
"""Smoke and contract tests for the cross_route_053 controlled harness."""
...
def run_tests():
    with tempfile.TemporaryDirectory(prefix="cross-route-053-harness-") as tmp_str:
        tmp=Path(tmp_str)
        completed,payload=_runner(_base_args(tmp/"dry"))
        assert completed.returncode == 0, payload
        assert payload["approved_chunks_count"] == 51
        ...
        assert payload["term_coverage"]["Maya"]["passed"] is True
        assert payload["term_coverage"]["chultun"]["passed"] is True
        assert payload["term_coverage"]["karst"]["passed"] is True
        assert payload["axis_coverage"]["geography / karst-hydrology"] is True
...
if __name__ == "__main__":
    run_tests()
    print("cross_route_053 controlled harness tests PASS")
