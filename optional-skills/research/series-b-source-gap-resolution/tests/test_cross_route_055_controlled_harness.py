#!/usr/bin/env python3
"""Smoke and contract tests for the cross_route_055 controlled harness."""
...
def run_tests():
    with tempfile.TemporaryDirectory(prefix="cross-route-055-harness-") as tmp_str:
        tmp=Path(tmp_str)
        completed,payload=_runner(_base_args(tmp/"dry"))
        assert completed.returncode == 0, payload
        assert payload["approved_chunks_count"] == 190
        ...
        assert payload["term_coverage"]["Ryukyu"]["passed"] is True
        assert payload["term_coverage"]["proxy"]["passed"] is True
        assert payload["axis_coverage"]["art_architecture_context_supplementary"] is True
...
if __name__ == "__main__":
    run_tests()
    print("cross_route_055 controlled harness tests PASS")
