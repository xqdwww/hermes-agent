#!/usr/bin/env python3
"""Safe API tests for the rel_space_036 controlled harness."""
...
from rel_space_036_alias_source_guard import check_text
...
def run_tests():
    ...
    assert "wrong_scope_cases" not in check_text(f"{CASE_ID} Heaven altar circular concentric platform")
    assert "listing_or_planning" in check_text(f"{CASE_ID} hotel booking ticket itinerary")
    assert check_text(f"{CASE_ID} Heaven altar circular concentric platform ritual spatial") == []
    with tempfile.TemporaryDirectory(prefix="rel-space-036-contracts-") as tmp_str:
        tmp=Path(tmp_str)
        completed,payload=_runner(_base_args(tmp/"dry"))
        assert completed.returncode == 0, payload
        completed,payload=_runner(["--case-id", "rel_space_031", "--handoff-manifest", str(HANDOFF), "--approved-chunks", str(CHUNKS), "--output-dir", str(tmp/"bad"), "--no-production-default", "--no-baseline-update", "--no-full-series-b"])
...
if __name__ == "__main__":
    run_tests()
    print("rel_space_036 safe API contracts tests PASS")
