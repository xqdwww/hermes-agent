...
EXPECTED_OFFICIAL_BASELINE_COMMIT = "45d91f96bec3ec64c7c8e47ef3aeb04691b8f3bf"
EXPECTED_OFFICIAL_BASELINE_TAG = "travel-series-b-official-56of60-human-reviewed-2026-06-25"
...
REQUIRED_NEWLY_COUNTABLE_CASES = {"hist_arch_022"}
...
REQUIRED_BASELINE_TRACE_VALUES = {"31/60", "39/60", "44/60", "50/60", EXPECTED_PREVIOUS_BASELINE, EXPECTED_BASELINE}
...
def _validate_current_baseline_metadata(payload: dict[str, Any], *, context: str) -> None:
    ...
    if payload.get("official_baseline_commit") != EXPECTED_OFFICIAL_BASELINE_COMMIT:
        raise ProductionTargetLayerError(f"{context}_BASELINE_COMMIT_INVALID", "metadata must reference the official 56/60 baseline commit")
    if payload.get("official_baseline_tag") != EXPECTED_OFFICIAL_BASELINE_TAG:
        raise ProductionTargetLayerError(f"{context}_BASELINE_TAG_INVALID", "metadata must reference the official 56/60 baseline tag")
