#!/usr/bin/env python3
"""Explicit-only nat_eco_047 single-case controlled harness."""
...
from series_b_nat_eco_047_artifact_contract import (
    NatEco047ArtifactContractError,
    validate_nat_eco_047_artifact_contract,
)
from series_b_nat_eco_047_controlled_auditor import ControlledAuditError, audit_nat_eco_047_dossier
from series_b_nat_eco_047_result_schema import CASE_ID, EXECUTION_FALSE_FLAGS
from series_b_nat_eco_047_source_grounded_builder import SourceGroundedBuilderError, build_nat_eco_047_dossier
from series_b_nat_eco_047_source_packet_exporter import (
    SourcePacketExportError,
    export_nat_eco_047_controlled_source_packet,
    validate_nat_eco_047_handoff_inputs,
)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Validate or execute nat_eco_047 single-case controlled dry-run.")
...
def _result_enum_for_error(code: str) -> str:
    ...
    if "CONTEXT" in code:
        return "BLOCKED_CONTEXT_PRIMARY_EVIDENCE_REQUIRED"
...
def _validate_inputs(args: argparse.Namespace) -> tuple[dict[str, object], dict[str, object], Path]:
    if args.case_id != CASE_ID:
        raise ValueError("CASE_ID_MISMATCH: case id must be nat_eco_047")
...
def _write_manifest_used(output_dir: Path, manifest: dict[str, object]) -> str:
    path = output_dir / "nat_eco_047_controlled_manifest_used.json"
    path.write_text(
        json.dumps(
            {
                "case_id": CASE_ID,
...
def _execute(args: argparse.Namespace, *, manifest: dict[str, object], output_dir: Path) -> tuple[int, dict[str, object]]:
    ...
    try:
        source_packet = export_nat_eco_047_controlled_source_packet(
        ...
        builder = build_nat_eco_047_dossier(
            case_id=args.case_id,
        ...
        audit = audit_nat_eco_047_dossier(
            case_id=args.case_id,
        ...
        contract = validate_nat_eco_047_artifact_contract(output_dir)
    ...
    except NatEco047ArtifactContractError as exc:
    ...
    payload = {
        ...
        "supplemental_locator_caveat_preserved": result.get("supplemental_locator_caveat_preserved"),
...
def run(args: argparse.Namespace) -> tuple[int, dict[str, object]]:
    ...
    except (NatEco047GuardError, SourcePacketExportError) as exc:
    ...
    return 0, {
        ...
        "case_id": CASE_ID,
        "approved_chunks_count": len(chunks_payload.get("chunks", [])),
        "binding_caveat_preserved": True,
        "supplemental_locator_caveat_preserved": True,
