#!/usr/bin/env python3
"""Explicit-only rel_space_030 single-case controlled harness."""
...
from series_b_rel_space_030_artifact_contract import RelSpace030ArtifactContractError, validate_rel_space_030_artifact_contract
from series_b_rel_space_030_controlled_auditor import ControlledAuditError, audit_rel_space_030_dossier
from series_b_rel_space_030_result_schema import CASE_ID, EXECUTION_FALSE_FLAGS
from series_b_rel_space_030_source_grounded_builder import SourceGroundedBuilderError, build_rel_space_030_dossier
from series_b_rel_space_030_source_packet_exporter import (
    SourcePacketExportError,
    export_rel_space_030_controlled_source_packet,
    validate_rel_space_030_handoff_inputs,
)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Validate or execute rel_space_030 single-case controlled dry-run.")
...
def _result_enum_for_error(code: str) -> str:
    ...
    if "MOUNT_MERU" in code:
        return "BLOCKED_MOUNT_MERU_PRIMARY_EVIDENCE_REQUIRED"
    if "SACRED_GEOMETRY" in code:
        return "BLOCKED_SACRED_GEOMETRY_EXACT_PHRASE_REQUIRED"
...
def _validate_inputs(args: argparse.Namespace) -> tuple[dict[str, object], dict[str, object], Path]:
    if args.case_id != CASE_ID:
        raise ValueError("CASE_ID_MISMATCH: case id must be rel_space_030")
...
def _write_manifest_used(output_dir: Path, manifest: dict[str, object]) -> str:
    path = output_dir / "rel_space_030_controlled_manifest_used.json"
    path.write_text(
        json.dumps(
            {"case_id": CASE_ID, "controlled_real_mode": True, "mock_artifact": False, "manifest": manifest, **EXECUTION_FALSE_FLAGS},
            indent=2,
            sort_keys=True,
        )
        + "\n",
...
def _execute(args: argparse.Namespace, *, manifest: dict[str, object], output_dir: Path) -> tuple[int, dict[str, object]]:
    ...
    try:
        source_packet = export_rel_space_030_controlled_source_packet(
        ...
        builder = build_rel_space_030_dossier(
            case_id=args.case_id,
        ...
        audit = audit_rel_space_030_dossier(
            case_id=args.case_id,
        ...
        contract = validate_rel_space_030_artifact_contract(output_dir)
    ...
    except RelSpace030ArtifactContractError as exc:
    ...
    payload = {
        ...
        "mount_meru_context_caveat_preserved": result.get("mount_meru_context_caveat_preserved"),
        "sacred_geometry_equivalent_caveat_preserved": result.get("sacred_geometry_equivalent_caveat_preserved"),
...
def run(args: argparse.Namespace) -> tuple[int, dict[str, object]]:
    ...
    except (RelSpace030GuardError, SourcePacketExportError) as exc:
    ...
    return 0, {
        ...
        "mount_meru_context_caveat_preserved": True,
        "sacred_geometry_equivalent_caveat_preserved": True,
