#!/usr/bin/env python3
"""Deterministic controlled auditor for cross_route_052 dry-runs."""
...
AUDIT_TRACE_FILENAME = "cross_route_052_controlled_audit_trace.json"
ALIAS_GUARD_FILENAME = "cross_route_052_alias_source_guard_report.md"
CONTAMINATION_FILENAME = "cross_route_052_contamination_check.md"
SUMMARY_FILENAME = "cross_route_052_controlled_execution_summary.md"
RESULT_FILENAME = "cross_route_052_controlled_execution_result.json"


class ControlledAuditError(ValueError):
    """Raised when the cross_route_052 controlled auditor must fail closed."""
...
def _term_checks(text: str) -> dict[str, dict[str, Any]]:
    checks = {
        ...
        "cave architecture": ["cave architecture"],
        "Buddhist cave temple": ["buddhist cave temple"],
        "fresco": ["fresco"],
        ...
        "cliff stability": ["cliff stability"],
...
def _axis_checks(packet: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "art_architecture_book": "art_architecture_book" in axes,
        "archaeology_history_source": "archaeology_history_source" in axes,
        "conservation_source": "conservation_source" in axes,
        "geology_hydrology_source": "geology_hydrology_source" in axes,
        "context_sources": isinstance(context_chunks, list) and len(context_chunks) == 3,
...
def _hydrology_checks(packet: dict[str, Any], text: str) -> dict[str, Any]:
    lowered = text.lower()
    approved_chunks = [chunk for chunk in packet.get("chunks", []) if isinstance(chunk, dict)]
    context_chunks = [chunk for chunk in packet.get("context_or_alias_guard_chunks", []) if isinstance(chunk, dict)]
    approved_hydrology = [
        chunk.get("chunk_id")
    ...
    caveat = "hydrology is context-only" in lowered and "not independent primary evidence" in lowered
    near_support = "hydrogeology" in lowered and "groundwater" in lowered and "cliff stability" in lowered
    overcredit = any(
        phrase in lowered
        for phrase in (
            "hydrology is independent primary evidence",
            "hydrology primary evidence",
            "hydrology exact term is professional primary",
        )
    )
    return {
        "approved_hydrology_chunk_ids": approved_hydrology,
        "context_hydrology_chunk_ids": context_hydrology,
        "hydrology_caveat_in_text": caveat,
        "hydrogeology_near_support_in_text": near_support,
        "hydrology_overcredited": overcredit,
        "passed": not approved_hydrology and bool(context_hydrology) and caveat and near_support and not overcredit,
    }
...
def audit_cross_route_052_dossier(
    *,
    case_id: str,
    ...
    """Audit cross_route_052 controlled artifacts without production paths."""

    if case_id != CASE_ID:
        raise ControlledAuditError("CASE_ID_MISMATCH", "case_id must be cross_route_052")
    ...
    if packet.get("case_id") != CASE_ID or manifest.get("case_id") != CASE_ID:
        raise ControlledAuditError("CASE_ID_MISMATCH", "audit inputs must be cross_route_052")
    ...
    hydrology_checks = _hydrology_checks(packet, text)
    source_binding = _source_binding(text, packet)
    quality = _quality(text, term_checks, axis_checks, section_checks, binding_checks, hydrology_checks, source_binding)

    if guard_violations:
        result_enum = "BLOCKED_GUARD_VIOLATION"
    elif not binding_checks["passed"]:
        result_enum = "BLOCKED_BINDING_INSUFFICIENT"
    elif not hydrology_checks["passed"]:
        result_enum = "BLOCKED_HYDROLOGY_PRIMARY_EVIDENCE_REQUIRED"
    ...
    audit_trace = {
        ...
        "audit_adapter_version": "cross_route_052.controlled_auditor.v1",
    ...
    result = {
        ...
        "hydrology_caveat_preserved": hydrology_checks["passed"],
    ...
    paths[ALIAS_GUARD_FILENAME].write_text(
        "# cross_route_052 alias/source guard report\n\n"
        "case_id: cross_route_052\n"
        "mock_audit_output: false\n"
        "accepted_primary_sources: Getty Cave85 Conservation; Wu Spatial Dunhuang; Wang Mogao Cliff Reinforcement\n"
        f"status: {'PASS' if not guard_violations else 'FAIL'}\n"
        f"violations: {guard_violations}\n",
        encoding="utf-8",
    )
    paths[CONTAMINATION_FILENAME].write_text(
        "# cross_route_052 contamination check\n\n"
        "case_id: cross_route_052\n"
        "mock_audit_output: false\n"
        f"status: {'PASS' if not guard_violations else 'FAIL'}\n"
        f"violations: {guard_violations}\n"
    ...
    paths[SUMMARY_FILENAME].write_text(
        "# cross_route_052 controlled execution summary\n\n"
        "case_id: cross_route_052\n"
        "single_case_controlled_dryrun_evidence: true\n"
        "official_baseline_update_performed: false\n"
        "full_series_b_run_performed: false\n"
        ...
        f"hydrology_caveat_preserved: {str(result['hydrology_caveat_preserved']).lower()}\n"
