#!/usr/bin/env python3
"""Explicit-only obj_art_007 single-case controlled harness."""
...
from series_b_obj_art_007_artifact_contract import (
    ObjArt007ArtifactContractError,
    validate_obj_art_007_artifact_contract,
)
from series_b_obj_art_007_controlled_auditor import ControlledAuditError, audit_obj_art_007_dossier
from series_b_obj_art_007_result_schema import CASE_ID, EXECUTION_FALSE_FLAGS
from series_b_obj_art_007_source_grounded_builder import SourceGroundedBuilderError, build_obj_art_007_dossier
from series_b_obj_art_007_source_packet_exporter import (
    ...
    validate_obj_art_007_handoff_inputs,
)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Validate or execute obj_art_007 single-case controlled dry-run.")
...
def _result_enum_for_error(code: str) -> str:
    ...
    if "PAGE" in code:
        return "BLOCKED_EXACT_PAGE_REQUIRED"
...
def _validate_inputs(args: argparse.Namespace) -> tuple[dict[str, object], dict[str, object], Path]:
    if args.case_id != CASE_ID:
        raise ValueError("CASE_ID_MISMATCH: case id must be obj_art_007")
...
def _write_manifest_used(output_dir: Path, manifest: dict[str, object]) -> str:
    path = output_dir / "obj_art_007_controlled_manifest_used.json"
    path.write_text(
        json.dumps(
            {
                "case_id": CASE_ID,
...
def _execute(args: argparse.Namespace, *, manifest: dict[str, object], output_dir: Path) -> tuple[int, dict[str, object]]:
    ...
    try:
        source_packet = export_obj_art_007_controlled_source_packet(
        ...
        builder = build_obj_art_007_dossier(
            case_id=args.case_id,
        ...
        audit = audit_obj_art_007_dossier(
            case_id=args.case_id,
        ...
        contract = validate_obj_art_007_artifact_contract(output_dir)
    ...
    except ObjArt007ArtifactContractError as exc:
    ...
    payload = {
        ...
        "page_binding_caveat_preserved": result.get("page_binding_caveat_preserved"),
        "precise_page_number_claimed": result.get("precise_page_number_claimed"),
...
def run(args: argparse.Namespace) -> tuple[int, dict[str, object]]:
    ...
    except (ObjArt007GuardError, SourcePacketExportError) as exc:
