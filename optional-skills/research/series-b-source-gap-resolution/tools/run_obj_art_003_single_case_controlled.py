#!/usr/bin/env python3
"""Explicit-only obj_art_003 single-case controlled harness."""
...
from series_b_obj_art_003_artifact_contract import (
    ObjArt003ArtifactContractError,
    validate_obj_art_003_artifact_contract,
)
from series_b_obj_art_003_controlled_auditor import ControlledAuditError, audit_obj_art_003_dossier
from series_b_obj_art_003_result_schema import CASE_ID, EXECUTION_FALSE_FLAGS
from series_b_obj_art_003_source_grounded_builder import SourceGroundedBuilderError, build_obj_art_003_dossier
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Validate or execute obj_art_003 single-case controlled dry-run.")
...
def _result_enum_for_error(code: str) -> str:
    if code in {
        "BLOCKED_CASE_RULE_TRACE_UNCLEAR",
        "BLOCKED_NEEDS_ARCHAEOLOGY_SOURCE",
...
def _validate_inputs(args: argparse.Namespace) -> tuple[dict[str, object], dict[str, object], Path]:
    if args.case_id != CASE_ID:
        raise ValueError("CASE_ID_MISMATCH: case id must be obj_art_003")
    ...
    if manifest.get("case_id") != CASE_ID or chunks_payload.get("case_id") != CASE_ID:
        raise ValueError("CASE_ID_MISMATCH: handoff inputs must be obj_art_003")
    if manifest.get("archaeology_axis_decision") != "ARCHAEOLOGY_AXIS_NOT_HARD_REQUIRED_CONFIRMED":
        raise ValueError("BLOCKED_CASE_RULE_TRACE_UNCLEAR: archaeology hard-axis trace is not confirmed")
    policy = manifest.get("production_path_policy")
    if not isinstance(policy, dict):
        raise ValueError("HANDOFF_MANIFEST_MALFORMED: production_path_policy is required")
    ...
    if not isinstance(chunks, list) or not chunks:
        raise ValueError("APPROVED_CHUNKS_MALFORMED: approved_chunks must be a non-empty list")
    validate_chunks(chunks)
    return manifest, chunks_payload, output_dir


def _write_manifest_used(output_dir: Path, manifest: dict[str, object]) -> str:
    path = output_dir / "obj_art_003_controlled_manifest_used.json"
    path.write_text(
        json.dumps(
            {
                "case_id": CASE_ID,
...
def _execute(args: argparse.Namespace, *, manifest: dict[str, object], output_dir: Path) -> tuple[int, dict[str, object]]:
    ...
    try:
        source_packet = export_obj_art_003_controlled_source_packet(
        ...
        builder = build_obj_art_003_dossier(
            case_id=args.case_id,
        ...
        audit = audit_obj_art_003_dossier(
            case_id=args.case_id,
        ...
        contract = validate_obj_art_003_artifact_contract(output_dir)
    ...
    except ObjArt003ArtifactContractError as exc:
...
def run(args: argparse.Namespace) -> tuple[int, dict[str, object]]:
    ...
    return 0, {
        ...
        "case_id": CASE_ID,
        "approved_chunks_count": len(chunks_payload.get("approved_chunks", [])),
        "case_rule_trace_decision": manifest.get("archaeology_axis_decision"),
