#!/usr/bin/env python3
"""Explicit-only cross_route_052 single-case controlled harness."""
...
from series_b_cross_route_052_artifact_contract import (
    CrossRoute052ArtifactContractError,
    validate_cross_route_052_artifact_contract,
)
from series_b_cross_route_052_controlled_auditor import ControlledAuditError, audit_cross_route_052_dossier
from series_b_cross_route_052_result_schema import CASE_ID, EXECUTION_FALSE_FLAGS
from series_b_cross_route_052_source_grounded_builder import SourceGroundedBuilderError, build_cross_route_052_dossier
from series_b_cross_route_052_source_packet_exporter import (
    SourcePacketExportError,
    export_cross_route_052_controlled_source_packet,
    validate_cross_route_052_handoff_inputs,
)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Validate or execute cross_route_052 single-case controlled dry-run.")
...
def _result_enum_for_error(code: str) -> str:
    ...
    if "HYDROLOGY" in code:
        return "BLOCKED_HYDROLOGY_PRIMARY_EVIDENCE_REQUIRED"
...
def _validate_inputs(args: argparse.Namespace) -> tuple[dict[str, object], dict[str, object], Path]:
    if args.case_id != CASE_ID:
        raise ValueError("CASE_ID_MISMATCH: case id must be cross_route_052")
    ...
    primary, supplemental, _context = validate_cross_route_052_handoff_inputs(
        manifest=manifest,
...
def _write_manifest_used(output_dir: Path, manifest: dict[str, object]) -> str:
    path = output_dir / "cross_route_052_controlled_manifest_used.json"
    path.write_text(
        json.dumps(
            {
                "case_id": CASE_ID,
...
def _execute(args: argparse.Namespace, *, manifest: dict[str, object], output_dir: Path) -> tuple[int, dict[str, object]]:
    ...
    try:
        source_packet = export_cross_route_052_controlled_source_packet(
        ...
        builder = build_cross_route_052_dossier(
            case_id=args.case_id,
        ...
        audit = audit_cross_route_052_dossier(
            case_id=args.case_id,
        ...
        contract = validate_cross_route_052_artifact_contract(output_dir)
    ...
    except CrossRoute052ArtifactContractError as exc:
    ...
    payload = {
        ...
        "hydrology_caveat_preserved": result.get("hydrology_caveat_preserved"),
...
def run(args: argparse.Namespace) -> tuple[int, dict[str, object]]:
    ...
    except (CrossRoute052GuardError, SourcePacketExportError) as exc:
    ...
    return 0, {
        ...
        "approved_supplemental_chunks_count": len(chunks_payload.get("approved_supplemental_chunks", [])),
