#!/usr/bin/env python3
"""Explicit-only nat_eco_041 single-case controlled harness."""
...
from series_b_nat_eco_041_artifact_contract import (
    NatEco041ArtifactContractError,
    validate_nat_eco_041_artifact_contract,
)
from series_b_nat_eco_041_controlled_auditor import ControlledAuditError, audit_nat_eco_041_dossier
from series_b_nat_eco_041_result_schema import CASE_ID, EXECUTION_FALSE_FLAGS
from series_b_nat_eco_041_source_grounded_builder import SourceGroundedBuilderError, build_nat_eco_041_dossier
from series_b_nat_eco_041_source_packet_exporter import (
    SourcePacketExportError,
    export_nat_eco_041_controlled_source_packet,
)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Validate or execute nat_eco_041 single-case controlled dry-run.")
...
def _result_enum_for_error(code: str) -> str:
    ...
    if "GUARD" in code or "CONTAMINATION" in code:
...
def _policy_ok(payload: dict[str, object], *, label: str) -> None:
    locks = payload.get("policy_locks")
    if not isinstance(locks, dict):
        raise ValueError(f"HANDOFF_MANIFEST_MALFORMED: {label} policy_locks is required")
    if locks.get("production_default_loader_enabled") is not False:
        raise ValueError("BLOCKED_PRODUCTION_DEFAULT_RISK: production default enabled")
    if locks.get("official_baseline_update_enabled") is not False:
        raise ValueError("BLOCKED_BASELINE_UPDATE_RISK: baseline update enabled")
    if locks.get("full_series_b_enabled") is not False:
        raise ValueError("BLOCKED_FULL_SERIES_B_RISK: full Series B enabled")


def _validate_inputs(args: argparse.Namespace) -> tuple[dict[str, object], dict[str, object], Path]:
    if args.case_id != CASE_ID:
        raise ValueError("CASE_ID_MISMATCH: case id must be nat_eco_041")
    ...
    if manifest.get("case_id") != CASE_ID or chunks_payload.get("case_id") != CASE_ID:
        raise ValueError("CASE_ID_MISMATCH: handoff inputs must be nat_eco_041")
    if manifest.get("formal_ready_decision") != "NAT_ECO_041_FORMAL_READY_REVIEW_APPROVED_WITH_BINDING_CAVEAT":
        raise ValueError("HANDOFF_MANIFEST_MALFORMED: formal-ready decision must preserve binding caveat")
    _policy_ok(manifest, label="manifest")
    _policy_ok(chunks_payload, label="chunks")
    professional = chunks_payload.get("professional_chunks")
    context = chunks_payload.get("context_chunks")
    if not isinstance(professional, list) or len(professional) != 13:
        raise ValueError("APPROVED_CHUNKS_MALFORMED: expected 13 professional chunks")
    if not isinstance(context, list) or len(context) != 28:
        raise ValueError("APPROVED_CHUNKS_MALFORMED: expected 28 context chunks")
    if chunks_payload.get("excluded_rejected_deferred_chunks_included") is not False:
        raise ValueError("APPROVED_CHUNKS_MALFORMED: rejected/deferred chunks are included")
    validate_chunks([*professional, *context])
    return manifest, chunks_payload, output_dir


def _write_manifest_used(output_dir: Path, manifest: dict[str, object]) -> str:
    path = output_dir / "nat_eco_041_controlled_manifest_used.json"
    path.write_text(
        json.dumps(
            {
                "case_id": CASE_ID,
...
def _execute(args: argparse.Namespace, *, manifest: dict[str, object], output_dir: Path) -> tuple[int, dict[str, object]]:
    ...
    try:
        source_packet = export_nat_eco_041_controlled_source_packet(
        ...
        builder = build_nat_eco_041_dossier(
            case_id=args.case_id,
        ...
        audit = audit_nat_eco_041_dossier(
            case_id=args.case_id,
        ...
        contract = validate_nat_eco_041_artifact_contract(output_dir)
    ...
    except NatEco041ArtifactContractError as exc:
    ...
    payload = {
        ...
        "dune_migration_caveat_preserved": result.get("dune_migration_caveat_preserved"),
...
def run(args: argparse.Namespace) -> tuple[int, dict[str, object]]:
    try:
        ...
        return _json_error("BLOCKED_GUARD_VIOLATION", str(exc))
    ...
    return 0, {
        ...
        "case_id": CASE_ID,
        "professional_chunks_count": len(chunks_payload.get("professional_chunks", [])),
        "context_chunks_count": len(chunks_payload.get("context_chunks", [])),
