...
def test_structure_mapper_canonical_binding_and_output(tmp_path: Path):
    ...
    codex_stage = research["plan"]["stages"][2]
    l4 = research["plan"]["stages"][4]
    assert l1["owner"] == GEMINI_HIGH
    assert l1["model"] == GEMINI_HIGH
    assert l1["executor_model"] == GEMINI_HIGH
    assert codex_stage["owner"] == "Codex executor"
    assert codex_stage["model"] == "Codex"
    assert codex_stage["executor_model"] == "Codex"
...
def test_l2_5_codex_executor_smoke_writes_compatibility_protocol_files(tmp_path: Path):
    ...
    assert stages[-1]["owner"] == "Codex executor"
    assert stages[-1]["executor_model"] == "Codex"
    assert stages[-1]["status"] == "codex-executor-smoke"
