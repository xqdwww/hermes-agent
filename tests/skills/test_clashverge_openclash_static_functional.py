...
def test_official_hk_policy_overrides_service_screens_without_name_inference() -> None:
    ...
    assert "neutral-country-unknown" in selections["gpt"]["automatic"]
    assert "neutral-country-unknown" in selections["gemini"]["automatic"]
...
def test_functional_loader_accepts_only_complete_attributed_rrc_v8(
    tmp_path: Path,
) -> None:
    path = tmp_path / "regioncheck-v8.json"
    base_result = {
